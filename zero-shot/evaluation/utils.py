...
def parse_concept_checkpoint_paths(epoch_str, base_path):
    
    if epoch_str == "all":
        pth = glob.glob(os.path.join(base_path, "concept_network_ckpt_epoch_*.pth"))
        pth = sorted(pth, key=lambda p: int(os.path.basename(p).replace("concept_network_ckpt_epoch_", "").replace(".pth", "")), reverse=True)
        
    else:
        epochs = epoch_str.strip().split(',')
        epochs = [int(e.strip()) for e in epochs]
        epochs = sorted(epochs)[::-1]
        pth = [os.path.join(base_path, "concept_network_ckpt_epoch_{}.pth".format(e)) for e in epochs]
        
    
    return pth

def inference(cfg, paths, eval_cls, batch_size, model, device, image_size,eval_attr):
    pred_attr = []
    pred_latent = []
    ...
    #         every_cls_attr.append(eval_attr[indx])
    #     indx +=1
    #     all_image_attr.append(every_cls_attr)
    
    for item in paths:
       
        for num in range(len(item)):
            all_image_attr.append(eval_attr[indx])
        indx +=1
        
    
    ################################
    evaluate_dataset = EvaluationFactory(pred_all_paths, cfg.test.imload_mode, image_size,all_image_attr)
    ...
    with torch.no_grad():
        ...
        for ims, attr_label in tqdm(evaluate_dataloader):
            
            ims = ims.to(device)
            attr_label = attr_label.to(device)
            ims = ims.view(-1, 3, ims.size()[-2], ims.size()[-1])
            
            # pred_attr_batch, attr_cls_batch = model(ims, attr_label)
            # pred_attr.append(pred_attr_batch.detach().cpu().numpy())
            # pred_latent.append(pred_attr_batch.detach().cpu().numpy())
