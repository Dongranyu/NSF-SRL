...
class LFGAAResNet101(torch.nn.Module):
    ...
    def forward(self, x, attr_label):#x=[batch,3,224,224]
        ...
        x = F.relu(self.fc4(feature))#[batch,170=85*2]#85
     
        attr = self.bn1(x[:, :self.k])#[batch, 85] select the former 85
        #latent = self.bn2(x[:, self.k:])##[batch, 85] select the latter 85

        feats_0 = self.extract_0(conv1_layer)#[batch, 85, 14,14]
        feats_1 = self.extract_1(conv2_layer)#[batch, 85, 14,14]
        feats_2 = self.extract_2(conv3_layer)#[batch, 85, 14,14]
        ...
        p_0 = self.attention_sublayers(feats_0, self.fc0, attr_label)
