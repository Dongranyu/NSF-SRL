...
class Dataset:
    def __init__(self, data_root, shuffle_sampling= False,load_method=0):

        
        fact_ls, rule_ls, query_ls = preprocess_kinship(joinpath(data_root, 'relations_SS.txt'),
                                                        joinpath(data_root, 'facts_PS.txt'),
                                                        joinpath(data_root, 'rules_SS.txt'),
                                                        joinpath(data_root, 'queries_PS.txt'))
        valid_ls = []
    ...
    def get_batch_by_q(self, batchsize, observed_prob=1.0, validation=False):
        ...
        for val, pred_name, consts in fact_ls:
            ...
            for rule_i, rule in enumerate(self.rule_ls):
                ...
                sub = [None] * len(rule.rule_vars)  # substitutions
                vn0, = rule.atom_ls[-1].var_name_ls
